...
class GraphormerLayer(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.ffn = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=feat_size, out_features=hidden_size),
            activation,
            paddle.nn.Dropout(p=dropout),
            paddle.nn.Linear(in_features=hidden_size, out_features=feat_size),
            paddle.nn.Dropout(p=dropout),
        )
        self.dropout = paddle.nn.Dropout(p=dropout)
        self.attn_layer_norm = paddle.nn.LayerNorm(normalized_shape=feat_size)
        self.ffn_layer_norm = paddle.nn.LayerNorm(normalized_shape=feat_size)
