...
class DegreeEncoder(paddle.nn.Layer):
    ...
    def __init__(self, max_degree, embedding_dim, direction="both"):
        ...
        if direction == "both":
            self.encoder1 = paddle.nn.Embedding(
                num_embeddings=max_degree + 1,
                embedding_dim=embedding_dim,
                padding_idx=0,
            )
            self.encoder2 = paddle.nn.Embedding(
                num_embeddings=max_degree + 1,
                embedding_dim=embedding_dim,
                padding_idx=0,
            )
        else:
            self.encoder = paddle.nn.Embedding(
                num_embeddings=max_degree + 1,
                embedding_dim=embedding_dim,
                padding_idx=0,
            )
    ...
    def forward(self, degrees):
        ...
        """
        degrees = paddle.clip(x=degrees, min=0, max=self.max_degree)
