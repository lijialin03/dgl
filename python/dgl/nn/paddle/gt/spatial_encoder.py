...
class SpatialEncoder(paddle.nn.Layer):
    ...
    def __init__(self, max_dist, num_heads=1):
        ...
        self.embedding_table = paddle.nn.Embedding(num_embeddings=max_dist + 2, embedding_dim=num_heads, padding_idx=0)
    ...
    def forward(self, dist):
        ...
        """
        spatial_encoding = self.embedding_table(paddle.clip(x=dist, min=-1, max=self.max_dist) + 1)
...
class SpatialEncoder3d(paddle.nn.Layer):
    ...
    def __init__(self, num_kernels, num_heads=1, max_node_type=100):
        ...
        self.means = paddle.base.framework.EagerParamBase.from_tensor(tensor=paddle.empty(shape=num_kernels))
        self.stds = paddle.base.framework.EagerParamBase.from_tensor(tensor=paddle.empty(shape=num_kernels))
        self.linear_layer_1 = paddle.nn.Linear(in_features=num_kernels, out_features=num_kernels)
        self.linear_layer_2 = paddle.nn.Linear(in_features=num_kernels, out_features=num_heads)
        self.gamma = paddle.nn.Embedding(num_embeddings=2 * max_node_type + 3, embedding_dim=1, padding_idx=0)
        self.beta = paddle.nn.Embedding(num_embeddings=2 * max_node_type + 3, embedding_dim=1, padding_idx=0)
    ...
    def forward(self, coord, node_type=None):
        ...
        if node_type is None:
            ...
            src_node_type = node_type.unsqueeze(axis=-1).tile(repeat_times=[1, 1, N])
            tgt_node_type = node_type.unsqueeze(axis=1).tile(repeat_times=[1, N, 1])
