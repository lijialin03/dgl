...
class EGTLayer(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.edge_input = paddle.nn.Linear(
            in_features=edge_feat_size, out_features=num_heads
        )
        self.qkv_proj = paddle.nn.Linear(
            in_features=feat_size, out_features=feat_size * 3
        )
        self.gate = paddle.nn.Linear(
            in_features=edge_feat_size, out_features=num_heads
        )
        self.attn_dropout = paddle.nn.Dropout(p=attn_dropout)
        self.node_output = paddle.nn.Linear(
            in_features=feat_size, out_features=feat_size
        )
        ...
        if self.edge_update:
            self.edge_output = paddle.nn.Linear(
                in_features=num_heads, out_features=edge_feat_size
            )
            ...
            self.edge_ffn = paddle.nn.Sequential(
                ...
                paddle.nn.Linear(
                    in_features=edge_feat_size, out_features=edge_feat_size
                ),
                activation,
                paddle.nn.Linear(
                    in_features=edge_feat_size, out_features=edge_feat_size
    ...
    def forward(self, nfeat, efeat, mask=None):
        ...
        if mask is None:
            ...
            gates = paddle.nn.functional.sigmoid(
                x=gates + mask.unsqueeze(axis=-1)
            )
            attn_tild = (
                paddle.nn.functional.softmax(
                    x=attn_hat + mask.unsqueeze(axis=-1), axis=2
                )
                * gates
            )
