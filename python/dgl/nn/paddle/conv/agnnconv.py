...
class AGNNConv(paddle.nn.Layer):
    """Attention-based Graph Neural Network layer from `Attention-based Graph Neural Network for
    ...
    def __init__(
        ...
        if learn_beta:
            self.beta = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.to_tensor(data=[init_beta], dtype="float32")
            )
        else:
