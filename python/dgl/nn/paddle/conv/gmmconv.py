...
class GMMConv(paddle.nn.Layer):
    """Gaussian Mixture Model Convolution layer from `Geometric Deep
    ...
    def reset_parameters(self):
        ...
        init_Normal = paddle.nn.initializer.Normal(mean=0, std=0.1)
        init_Normal(self.mu.data)
        init_Constant = paddle.nn.initializer.Constant(value=1)
        init_Constant(self.inv_sigma.data)
        if self.bias is not None:
            init_Constant = paddle.nn.initializer.Constant(value=0.0)
            init_Constant(self.bias.data)
    ...
    def forward(self, graph, feat, pseudo):
        ...
        with graph.local_scope():
            ...
            rst = graph.dstdata["h"].sum(axis=1)
