...
class GCN2Conv(paddle.nn.Layer):
    """Graph Convolutional Network via Initial residual
    ...
    def __init__(
        ...
        self.weight1 = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.empty(shape=[self._in_feats, self._in_feats])
        )
        if self._project_initial_features:
            self.add_parameter(name="weight2", parameter=None)
        else:
        ...
        if self._bias:
            self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.to_tensor(data=self._in_feats, dtype="float32")
            )
        else:
    ...
    def reset_parameters(self):
        ...
        init_Normal(self.weight1)
        if not self._project_initial_features:
            init_Normal = paddle.nn.initializer.Normal()
            init_Normal(self.weight2)
        if self._bias:
        ...
        * Weight shape: :math:`(\\text{in_feats}, \\text{out_feats})`.
        ...
        with graph.local_scope():
            ...
            if edge_weight is None:
                degs = graph.in_degrees().to(feat).clip(min=1)
                norm = paddle.pow(x=degs, y=-0.5)
                norm = norm.to(feat.place).unsqueeze(axis=1)
            else:
            ...
            feat_0 = feat_0[: feat.shape[0]] * self.alpha
