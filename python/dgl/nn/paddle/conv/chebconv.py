...
class ChebConv(paddle.nn.Layer):
    ...
    def __init__(
        ...
        out_feats,
        k,
        activation=paddle.nn.functional.relu,
        bias=True,
    ):
    ...
    def forward(self, graph, feat, lambda_max=None):
        ...
        with graph.local_scope():
            ...
            if isinstance(lambda_max, list):
                lambda_max = paddle.to_tensor(data=lambda_max).to(feat)
            if lambda_max.dim() == 1:
                lambda_max = lambda_max.unsqueeze(axis=-1)
