...
class InvertibleCheckpoint(paddle.autograd.PyLayer):
        ...
        with paddle.no_grad():
            ...
            for element in inputs:
                if isinstance(element, paddle.Tensor):
                    ...
                    ctx.input_requires_grad.append(not element.stop_gradient)
                else:
                    x.append(element)
    ...
    def backward(ctx, *grad_outputs):
        ...
        with paddle.no_grad():
            ...
            x_new = paddle.empty(shape=[total_size], dtype=x.dtype)
            x_new[:] = paddle.reshape(inputs_inverted, [total_size])
            inputs[1] = x_new.reshape(x.shape)

        with paddle.set_grad_enabled(mode=True):
            detached_inputs = []
            for i, element in enumerate(inputs):
                if isinstance(element, paddle.Tensor):
                    element = element.detach()
                    element.stop_gradient = not ctx.input_requires_grad[i]
...
class GroupRevRes(paddle.nn.Layer):
    """Grouped reversible residual connections for GNNs, as introduced in
    ...
    def __init__(self, gnn_module, groups=2):
        ...
        self.gnn_modules = paddle.nn.LayerList()
    ...
    def _forward(self, g, x, *args):
        xs = paddle.chunk(x=x, chunks=self.groups, axis=-1)
        if len(args) == 0:
        ...
        for i in range(self.groups):
            ...
            ys.append(y_in)
        out = paddle.concat(x=ys, axis=-1)
        return out

    def _inverse(self, g, y, *args):
        ys = paddle.chunk(x=y, chunks=self.groups, axis=-1)
        if len(args) == 0:
            args_chunks = [()] * self.groups
        else:
            chunked_args = list(
