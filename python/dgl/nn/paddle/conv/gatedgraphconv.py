...
class GatedGraphConv(paddle.nn.Layer):
    """Gated Graph Convolution layer from `Gated Graph Sequence
    ...
    def reset_parameters(self):
        ...
        for linear in self.linears:
            ...
            init_XavierNormal(linear.weight)
            init_Constant = paddle.nn.initializer.Constant(value=0.0)
            init_Constant(linear.bias)
    ...
    def forward(self, graph, feat, etypes=None):
        ...
        with graph.local_scope():
            ...
            ), "not a homogeneous graph; convert it with to_homogeneous and pass in the edge type as argument"
            if self._n_etypes != 1:
                assert (
            ...
            zero_pad = paddle.zeros(
                shape=(
                    tuple(feat.shape)[0],
                    self._out_feats - tuple(feat.shape)[1],
                ),
                dtype=feat.dtype,
            )
            feat = paddle.concat(x=[feat, zero_pad], axis=-1)
