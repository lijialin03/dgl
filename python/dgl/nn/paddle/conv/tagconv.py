...
class TAGConv(paddle.nn.Layer):
    """Topology Adaptive Graph Convolutional layer from `Topology
    ...
    def __init__(self, in_feats, out_feats, k=2, bias=True, activation=None):
        ...
        self.lin = paddle.nn.Linear(
            in_features=in_feats * (self._k + 1),
    ...
    def reset_parameters(self):
        ...
        init_XavierNormal(self.lin.weight)
        ...
        with graph.local_scope():
            ...
            for _ in range(self._k):
                ...
                if edge_weight is None:
                    rst = rst * norm
                fstack.append(rst)
            rst = self.lin(paddle.concat(x=fstack, axis=-1))
