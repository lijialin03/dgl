...
class RadialPooling(paddle.nn.Layer):
    """Radial pooling from `Atomic Convolutional Networks for
    ...
    def forward(self, distances):
        ...
        cutoff_values = paddle.where(
            ...
            x=cos_values,
            y=paddle.zeros_like(x=cos_values),
        )
...
def reduce_func(nodes):
    ...
    dict mapping 'hv_new' to Float32 tensor of shape (V, K * T)
        ...
        (types of atomic number in the paper).
    """
    return {"hv_new": nodes.mailbox["m"].sum(axis=1)}


class AtomicConv(paddle.nn.Layer):
    """Atomic Convolution Layer from `Atomic Convolutional Networks for
    Predicting Protein-Ligand Binding Affinity <https://arxiv.org/abs/1703.10603>`__
    ...
    * :math:`\\gamma_{k}`, :math:`r_k` and :math:`c_k` are all learnable.
    ...
    def forward(self, graph, feat, distances):
        ...
        with graph.local_scope():
            radial_pooled_values = self.radial_pooling(distances).to(feat)
            if self.features_to_use is not None:
                feat = (feat == self.features_to_use).to(feat)
            graph.ndata["hv"] = feat
            graph.edata["he"] = radial_pooled_values.transpose(
                perm=transpose_aux_func(radial_pooled_values.ndim, 1, 0)
            ).squeeze(axis=-1)
            graph.update_all(msg_func, reduce_func)
            return graph.ndata["hv_new"].view(graph.num_nodes(), -1)
