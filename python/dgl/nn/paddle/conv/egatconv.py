...
class EGATConv(paddle.nn.Layer):
    ...
    def __init__(
        ...
        if isinstance(in_node_feats, tuple):
            self.fc_node_src = paddle.nn.Linear(
                in_features=self._in_src_node_feats,
                out_features=out_node_feats * num_heads,
                bias_attr=False,
            )
            self.fc_ni = paddle.nn.Linear(
                in_features=self._in_src_node_feats,
                out_features=out_edge_feats * num_heads,
                bias_attr=False,
            )
            self.fc_nj = paddle.nn.Linear(
                in_features=self._in_dst_node_feats,
                out_features=out_edge_feats * num_heads,
                bias_attr=False,
            )
        else:
        ...
        self.fc_fij = paddle.nn.Linear(
            in_features=in_edge_feats,
            out_features=out_edge_feats * num_heads,
            bias_attr=False,
        )
    ...
    def reset_parameters(self):
        ...
        init_XavierNormal(self.fc_node_src.weight)
        init_XavierNormal = paddle.nn.initializer.XavierNormal(gain=gain)
        init_XavierNormal(self.fc_ni.weight)
        init_XavierNormal = paddle.nn.initializer.XavierNormal(gain=gain)
        init_XavierNormal(self.fc_fij.weight)
        init_XavierNormal = paddle.nn.initializer.XavierNormal(gain=gain)
        init_XavierNormal(self.fc_nj.weight)
        init_XavierNormal = paddle.nn.initializer.XavierNormal(gain=gain)
        init_XavierNormal(self.attn)
    ...
    def forward(
        ...
        with graph.local_scope():
            ...
            f_out = paddle.nn.functional.leaky_relu(x=f_out)
