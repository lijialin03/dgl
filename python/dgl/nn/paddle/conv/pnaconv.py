...
def aggregate_mean(h):
    """mean aggregation"""
    return paddle.mean(x=h, axis=1)


def aggregate_max(h):
    """max aggregation"""
    return (paddle.max(x=h, axis=1), paddle.argmax(x=h, axis=1))[0]


def aggregate_min(h):
    """min aggregation"""
    return (paddle.min(x=h, axis=1), paddle.argmin(x=h, axis=1))[0]


def aggregate_sum(h):
    """sum aggregation"""
    return paddle.sum(x=h, axis=1)


def aggregate_std(h):
    """standard deviation aggregation"""
    return paddle.sqrt(x=aggregate_var(h) + 1e-30)


def aggregate_var(h):
    """variance aggregation"""
    h_mean_squares = paddle.mean(x=h * h, axis=1)
    h_mean = paddle.mean(x=h, axis=1)
    var = paddle.nn.functional.relu(x=h_mean_squares - h_mean * h_mean)
    return var
...
class PNAConvTower(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.batchnorm = paddle.nn.BatchNorm1D(num_features=out_size)
...
class PNAConv(paddle.nn.Layer):
    """Principal Neighbourhood Aggregation Layer from `Principal Neighbourhood Aggregation
    ...
    def __init__(
        ...
        if self.in_size != self.out_size:
            self.residual = False
        self.towers = paddle.nn.LayerList(
            sublayers=[
                PNAConvTower(
                    self.tower_in_size,
                    self.tower_out_size,
        ...
        self.mixing_layer = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=out_size, out_features=out_size),
            paddle.nn.LeakyReLU(),
        )
