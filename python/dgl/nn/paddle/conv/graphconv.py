...
class EdgeWeightNorm(paddle.nn.Layer):
    ...
    def forward(self, graph, edge_weight):
        ...
        with graph.local_scope():
            ...
            if len(tuple(edge_weight.shape)) > 1:
                raise DGLError(
            ...
            if self._norm == "both" and paddle.any(x=edge_weight <= 0).item():
                raise DGLError(
...
class GraphConv(paddle.nn.Layer):
    """Graph convolutional layer from `Semi-Supervised Classification with Graph Convolutional
        ...
        * Weight shape: :math:`(\\text{in_feats}, \\text{out_feats})`.
        ...
        with graph.local_scope():
            ...
            if self._norm in ["left", "both"]:
                degs = graph.out_degrees().to(feat_src).clip(min=1)
                if self._norm == "both":
                    norm = paddle.pow(x=degs, y=-0.5)
                else:
                    norm = 1.0 / degs
                shp = tuple(norm.shape) + (1,) * (feat_src.dim() - 1)
                norm = paddle.reshape(x=norm, shape=shp)
            ...
            if self._in_feats > self._out_feats:
                if weight is not None:
                    feat_src = paddle.matmul(x=feat_src, y=weight)
                ...
                rst = graph.dstdata["h"]
                if weight is not None:
                    rst = paddle.matmul(x=rst, y=weight)
            if self._norm in ["right", "both"]:
                degs = graph.in_degrees().to(feat_dst).clip(min=1)
                if self._norm == "both":
                    norm = paddle.pow(x=degs, y=-0.5)
                else:
                    norm = 1.0 / degs
                shp = tuple(norm.shape) + (1,) * (feat_dst.dim() - 1)
                norm = paddle.reshape(x=norm, shape=shp)
                rst = rst * norm
            if self.bias is not None:
                rst = rst + self.bias
