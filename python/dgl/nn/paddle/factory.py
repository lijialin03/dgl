...
from ...transforms import knn_graph
from ...transforms import radius_graph
from ...transforms import segmented_knn_graph
...
def pairwise_squared_distance(x):
    ...
    """
    x2s = (x * x).sum(axis=-1, keepdim=True)
    return (
        x2s
        + x2s.transpose(perm=transpose_aux_func(x2s.ndim, -1, -2))
        - 2 * x @ x.transpose(perm=transpose_aux_func(x.ndim, -1, -2))
    )


class KNNGraph(paddle.nn.Layer):
...
class RadiusGraph(paddle.nn.Layer):
    """Layer that transforms one point set into a bidirected graph with
