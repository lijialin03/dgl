...
class HeteroGraphConv(paddle.nn.Layer):
...
def _sum_reduce_func(inputs, dim):
    return paddle.sum(x=inputs, axis=dim)


def _mean_reduce_func(inputs, dim):
    return paddle.mean(x=inputs, axis=dim)


def _stack_agg_func(inputs, dsttype):
    if len(inputs) == 0:
        return None
    return paddle.stack(x=inputs, axis=1)


def _agg_func(inputs, dsttype, fn):
    if len(inputs) == 0:
        return None
    stacked = paddle.stack(x=inputs, axis=0)
...
class HeteroLinear(paddle.nn.Layer):
...
class HeteroEmbedding(paddle.nn.Layer):
    ...
    def reset_parameters(self):
        ...
        for typ in self.embeds.keys():
            ...
            init_XavierUniform(self.embeds[typ].weight)
