...
from ...backend import paddle as F
from ...base import dgl_warning
from ...readout import broadcast_nodes
from ...readout import max_nodes
from ...readout import mean_nodes
from ...readout import softmax_nodes
from ...readout import sum_nodes
from ...readout import topk_nodes
...
class SumPooling(paddle.nn.Layer):
...
class AvgPooling(paddle.nn.Layer):
...
class MaxPooling(paddle.nn.Layer):
...
class SortPooling(paddle.nn.Layer):
    ...
    def forward(self, graph, feat):
        ...
        """
        with graph.local_scope():
            feat, _ = paddle.sort(x=feat, axis=-1), paddle.argsort(x=feat, axis=-1)
            graph.ndata["h"] = feat
            ret = topk_nodes(graph, "h", self.k, sortby=-1)[0].view(-1, self.k * tuple(feat.shape)[-1])
            return ret


class GlobalAttentionPooling(paddle.nn.Layer):
    ...
    def forward(self, graph, feat, get_attention=False):
        ...
        with graph.local_scope():
            ...
            assert tuple(gate.shape)[-1] == 1, "The output of gate_nn should have size 1 at the last axis."
...
class Set2Set(paddle.nn.Layer):
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            h = paddle.zeros(shape=(self.n_layers, batch_size, self.input_dim), dtype=feat.dtype,), paddle.zeros(
                shape=(self.n_layers, batch_size, self.input_dim),
                dtype=feat.dtype,
            )
...
def _gen_mask(lengths_x, lengths_y, max_len_x, max_len_y):
    ...
    """
    x_mask = paddle.arange(end=max_len_x).unsqueeze(axis=0) < lengths_x.unsqueeze(axis=1)
    y_mask = paddle.arange(end=max_len_y).unsqueeze(axis=0) < lengths_y.unsqueeze(axis=1)
    mask = (x_mask.unsqueeze(axis=-1) & y_mask.unsqueeze(axis=-2)).unsqueeze(axis=1)
    return mask


class MultiHeadAttention(paddle.nn.Layer):
    """Multi-Head Attention block, used in Transformer, Set Transformer and so on
    ...
    def __init__(self, d_model, num_heads, d_head, d_ff, dropouth=0.0, dropouta=0.0):
        ...
        self.d_head = d_head
        self.d_ff = d_ff
        self.proj_q = paddle.nn.Linear(
            in_features=d_model,
            out_features=num_heads * d_head,
            bias_attr=False,
        )
        self.proj_k = paddle.nn.Linear(
            in_features=d_model,
            out_features=num_heads * d_head,
            bias_attr=False,
        )
        self.proj_v = paddle.nn.Linear(
            in_features=d_model,
            out_features=num_heads * d_head,
            bias_attr=False,
        )
        self.proj_o = paddle.nn.Linear(
            in_features=num_heads * d_head,
            out_features=d_model,
            bias_attr=False,
        )
        self.ffn = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=d_model, out_features=d_ff),
            paddle.nn.ReLU(),
            paddle.nn.Dropout(p=dropouth),
            paddle.nn.Linear(in_features=d_ff, out_features=d_model),
        )
        self.droph = paddle.nn.Dropout(p=dropouth)
        self.dropa = paddle.nn.Dropout(p=dropouta)
        self.norm_in = paddle.nn.LayerNorm(normalized_shape=d_model)
        self.norm_inter = paddle.nn.LayerNorm(normalized_shape=d_model)
        self.reset_parameters()
    ...
    def forward(self, x, mem, lengths_x, lengths_mem):
        ...
        device = x.place
        lengths_x = paddle.to_tensor(data=lengths_x, dtype="int64", place=device)
        lengths_mem = paddle.to_tensor(data=lengths_mem, dtype="int64", place=device)
        ...
        e = paddle.einsum("bxhd,byhd->bhxy", queries, keys)
        e = e / np.sqrt(self.d_head)
        mask = _gen_mask(lengths_x, lengths_mem, max_len_x, max_len_mem)
        e = e.masked_fill(mask=mask == 0, value=-float("inf"))
        alpha = paddle.nn.functional.softmax(x=e, axis=-1)
        alpha = alpha.masked_fill(mask=mask == 0, value=0.0)
        out = paddle.einsum("bhxy,byhd->bxhd", alpha, values)
        out = self.proj_o(out.contiguous().view(batch_size, max_len_x, self.num_heads * self.d_head))
...
class SetAttentionBlock(paddle.nn.Layer):
    """SAB block from `Set Transformer: A Framework for Attention-based
...
class InducedSetAttentionBlock(paddle.nn.Layer):
    """ISAB block from `Set Transformer: A Framework for Attention-based
    Permutation-Invariant Neural Networks <https://arxiv.org/abs/1810.00825>`__
    ...
    def __init__(self, m, d_model, num_heads, d_head, d_ff, dropouth=0.0, dropouta=0.0):
        ...
        self.d_model = d_model
        self.inducing_points = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.empty(shape=[m, d_model], dtype="float32")
        )
        self.mha = paddle.nn.LayerList(
            sublayers=[
                MultiHeadAttention(
                    d_model,
                    num_heads,
    ...
    def reset_parameters(self):
        """Reinitialize learnable parameters."""
        init_XavierUniform = paddle.nn.initializer.XavierUniform()
        init_XavierUniform(self.inducing_points)
    ...
    def forward(self, feat, lengths):
        ...
        query = self.inducing_points.tile(repeat_times=[batch_size, 1])
    ...
    def extra_repr(self):
        ...
        """
        shape_str = "({}, {})".format(
            tuple(self.inducing_points.shape)[0],
            tuple(self.inducing_points.shape)[1],
        )
        return "InducedVector: " + shape_str


class PMALayer(paddle.nn.Layer):
    """Pooling by Multihead Attention from `Set Transformer: A Framework for Attention-based
    Permutation-Invariant Neural Networks <https://arxiv.org/abs/1810.00825>`__
    ...
    def __init__(self, k, d_model, num_heads, d_head, d_ff, dropouth=0.0, dropouta=0.0):
        ...
        self.d_model = d_model
        self.seed_vectors = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.empty(shape=[k, d_model], dtype="float32")
        )
        self.mha = MultiHeadAttention(
            d_model,
            num_heads,
    ...
    def reset_parameters(self):
        """Reinitialize learnable parameters."""
        init_XavierUniform = paddle.nn.initializer.XavierUniform()
        init_XavierUniform(self.seed_vectors)
    ...
    def forward(self, feat, lengths):
        ...
        """
        batch_size = len(lengths)
        query = self.seed_vectors.tile(repeat_times=[batch_size, 1])
    ...
    def extra_repr(self):
        ...
        which will come into effect when printing the model.
        """
        shape_str = "({}, {})".format(tuple(self.seed_vectors.shape)[0], tuple(self.seed_vectors.shape)[1])
        return "SeedVector: " + shape_str


class SetTransformerEncoder(paddle.nn.Layer):
    """The Encoder module from `Set Transformer: A Framework for Attention-based
...
class SetTransformerDecoder(paddle.nn.Layer):
    """The Decoder module from `Set Transformer: A Framework for Attention-based
    Permutation-Invariant Neural Networks <https://arxiv.org/pdf/1810.00825.pdf>`__
...
class WeightAndSum(paddle.nn.Layer):
    ...
    def __init__(self, in_feats):
        ...
        self.atom_weighting = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=in_feats, out_features=1),
            paddle.nn.Sigmoid(),
        )
