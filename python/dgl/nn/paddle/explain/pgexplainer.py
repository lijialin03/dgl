...
from .... import NID
from .... import batch
from .... import khop_in_subgraph
from .... import to_homogeneous

__all__ = ["PGExplainer", "HeteroPGExplainer"]


class PGExplainer(paddle.nn.Layer):
    """PGExplainer from `Parameterized Explainer for Graph Neural Network
    ...
    def __init__(
        ...
        self.init_bias = 0.0
        self.elayers = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=self.num_features, out_features=64),
            paddle.nn.ReLU(),
            paddle.nn.Linear(in_features=64, out_features=1),
        )

    def set_masks(self, graph, edge_mask=None):
        """Set the edge mask that plays a crucial role to explain the
        ...
        self.edge_mask = self.edge_mask.to(graph.place)

    def clear_masks(self):
        """Clear the edge mask that play a crucial role to explain the
    ...
    def loss(self, prob, ori_pred):
        """The loss function that is used to learn the edge
        distribution.
    ...
    def concrete_sample(self, w, beta=1.0, training=True):
        ...
        if training:
            ...
            gate_inputs = paddle.nn.functional.sigmoid(x=w)
        return gate_inputs
    ...
    def explain_node(self, nodes, graph, feat, temperature=1.0, training=False, **kwargs):
        ...
        if training:
            ...
            self.clear_masks()
        return probs, edge_mask, batched_graph, batched_inverse_indices
...
class HeteroPGExplainer(PGExplainer):
    ...
    def explain_graph(self, graph, feat, temperature=1.0, training=False, **kwargs):
        ...
        col_emb = homo_embed[col.astype(dtype="int64")]
        row_emb = homo_embed[row.astype(dtype="int64")]
        emb = paddle.concat(x=[col_emb, row_emb], axis=-1)
        emb = self.elayers(emb)
        values = emb.reshape(-1)
        values = self.concrete_sample(values, beta=temperature, training=training)
        ...
        hetero_edge_mask = self._edge_mask_to_heterogeneous(
            edge_mask=edge_mask, homograph=homo_graph, heterograph=graph
        )
    ...
    def _edge_mask_to_heterogeneous(self, edge_mask, homograph, heterograph):
        """Convert an edge mask from homogeneous mappings built through
