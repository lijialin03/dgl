...
class SubgraphX(paddle.nn.Layer):
    """SubgraphX from `On Explainability of Graph Neural Networks via Subgraph
    ...
    def shapley(self, subgraph_nodes):
        """Compute Shapley value with Monte Carlo approximation.
        ...
        for _ in range(self.shapley_steps):
            ...
            exclude_mask = paddle.ones(shape=num_nodes)
            ...
            exclude_feat = self.feat * exclude_mask.unsqueeze(axis=1).to(device)
            include_feat = self.feat * include_mask.unsqueeze(axis=1).to(device)
            with paddle.no_grad():
                exclude_probs = paddle.nn.functional.softmax(
                    self.model(self.graph, exclude_feat, **self.kwargs), axis=-1
                )
                exclude_value = exclude_probs[:, self.target_class]
                include_probs = paddle.nn.functional.softmax(
                    self.model(self.graph, include_feat, **self.kwargs), axis=-1
                )
                include_value = include_probs[:, self.target_class]
            marginal_contributions.append(include_value - exclude_value)
        return paddle.concat(x=marginal_contributions).mean().item()

    def get_mcts_children(self, mcts_node):
        """Get the children of the MCTS node for the search.
    ...
    def explain_graph(self, graph, feat, target_class, **kwargs):
        """Find the most important subgraph from the original graph for the
        ...
        ), f"The number of nodes in the            graph {graph.num_nodes()} should be bigger than {self.node_min}."
        self.graph = graph
        self.feat = feat
...
class HeteroSubgraphX(paddle.nn.Layer):
    """SubgraphX from `On Explainability of Graph Neural Networks via Subgraph
