...
class GNNExplainer(paddle.nn.Layer):
    """GNNExplainer model from `GNNExplainer: Generating Explanations for
    ...
    def _init_masks(self, graph, feat):
        """Initialize learnable feature and edge mask.
    ...
    def explain_node(self, node_id, graph, feat, **kwargs):
        """Learn and return a node feature mask and subgraph that play a
        ...
        sg_nodes = sg.ndata[NID].astype(dtype="int64")
        sg_edges = sg.edata[EID].astype(dtype="int64")
        feat = feat[sg_nodes]
        for key, item in kwargs.items():
            if paddle.is_tensor(x=item) and item.shape[0] == num_nodes:
                item = item[sg_nodes]
            elif paddle.is_tensor(x=item) and item.shape[0] == num_edges:
                item = item[sg_edges]
            kwargs[key] = item
        with paddle.no_grad():
...
class HeteroGNNExplainer(paddle.nn.Layer):
    """GNNExplainer model from `GNNExplainer: Generating Explanations for
    ...
    def explain_node(self, ntype, node_id, graph, feat, **kwargs):
        """Learn and return node feature masks and a subgraph that play a
        crucial role to explain the prediction made by the GNN for node
    ...
    def explain_graph(self, graph, feat, **kwargs):
        """Learn and return node feature masks and edge masks that play a
        crucial role to explain the prediction made by the GNN for a graph.
