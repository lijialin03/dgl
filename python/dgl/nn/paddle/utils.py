...
def matmul_maybe_select(A, B):
    ...
    if A.dtype == "int64" and len(tuple(A.shape)) == 1:
        return B.index_select(axis=0, index=A)
    else:
        return paddle.matmul(x=A, y=B)
...
def bmm_maybe_select(A, B, index):
    ...
    """
    if A.dtype == "int64" and len(tuple(A.shape)) == 1:
        B = B.view(-1, tuple(B.shape)[2])
        flatidx = index * tuple(B.shape)[1] + A
        return B.index_select(axis=0, index=flatidx)
    else:
        BB = B.index_select(axis=0, index=index)
        return paddle.bmm(x=A.unsqueeze(axis=1), y=BB).squeeze()
...
class WeightBasis(paddle.nn.Layer):
    """Basis decomposition from `Modeling Relational Data with Graph
    ...
    def __init__(self, shape, num_bases, num_outputs):
        ...
        self.w_comp = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.empty(shape=[self.num_outputs, self.num_bases])
        )
        init_XavierUniform = paddle.nn.initializer.XavierUniform(
            gain=paddle.nn.initializer.calculate_gain(nonlinearity="relu")
        )
        init_XavierUniform(self.w_comp)
...
class JumpingKnowledge(paddle.nn.Layer):
    ...
    **concatenation**
    ...
    def forward(self, feat_list):
        ...
        """
        if self.mode == "cat":
            return paddle.concat(x=feat_list, axis=-1)
        elif self.mode == "max":
            return paddle.stack(x=feat_list, axis=-1).max(dim=-1)[0]
        else:
            stacked_feat_list = paddle.stack(x=feat_list, axis=1)
            alpha, _ = self.lstm(stacked_feat_list)
            alpha = self.att(alpha).squeeze(axis=-1)
            alpha = paddle.nn.functional.softmax(x=alpha, axis=-1)
            return (stacked_feat_list * alpha.unsqueeze(axis=-1)).sum(axis=1)


class LabelPropagation(paddle.nn.Layer):
    ...
    def forward(self, g, labels, mask=None):
        ...
        """
        with g.local_scope():
            if len(tuple(labels.shape)) > 1 and labels.shape[1] > 1:
                labels = labels.to("float32")
            else:
                labels = (
                    paddle.nn.functional.one_hot(
                        num_classes=labels.view(-1).max().item() + 1,
                        x=labels.view(-1),
                    )
                    .astype("int64")
                    .to("float32")
            ...
            for _ in range(self.k):
                ...
                if self.clamp:
                    y = y.clip_(min=0.0, max=1.0)
                if self.normalize:
                    y = paddle.nn.functional.normalize(x=y, p=1)
                if self.reset:
                    y[mask] = labels[mask]
            return y
